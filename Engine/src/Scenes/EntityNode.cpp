...
namespace Rapture
{
    ...
    void EntityNode::addChild(std::shared_ptr<EntityNode> childNode) {
        ...
        // Add pointer to children collection if not already present
         if (std::find_if(m_children.begin(), m_children.end(), [childNode](const std::weak_ptr<EntityNode>& ptr) {
             return ptr.lock() == childNode;
         }) == m_children.end()) {
    ...
    void EntityNode::removeChild(std::shared_ptr<EntityNode> childNode)
    {
        ...
        auto it = std::find_if(m_children.begin(), m_children.end(), [childNode](const std::weak_ptr<EntityNode>& ptr) {
            return ptr.lock() == childNode;
        });
    ...
    void EntityNode::setParent(std::shared_ptr<EntityNode> parentNode)
    {
        ...
        if (parentNode) {
             if (std::find_if(parentNode->m_children.begin(), parentNode->m_children.end(), [this](const std::weak_ptr<EntityNode>& ptr) {
                 return ptr.lock() == shared_from_this();
