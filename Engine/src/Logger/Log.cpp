...
namespace Rapture {
	...
	void Log::Init()
	{
		...
		auto defaultFileSink = std::make_shared<spdlog::sinks::rotating_file_sink_mt>(
			"logs/Rapture.log", 5 * 1024 * 1024, 3);
		...
		auto createLogger = [&](const std::string& name, LogCategory category) {
			...
			auto categorizedCallbackSink = std::make_shared<spdlog::sinks::callback_sink_mt>(
				[category](const spdlog::details::log_msg& msg) {
					Log::LogCallback(msg, category);
				});
		...
		s_DebugLogger = createLogger("DEBUG", LogCategory::Debug);
		...
		std::set<LogCategory> allCategories = {
			LogCategory::Core, LogCategory::Client, LogCategory::Debug,
			LogCategory::Render, LogCategory::Physics, LogCategory::Audio
		};
		s_FileSinkCategories["logs/Rapture.log"] = allCategories;
		
		// Create debug-specific log file
		EnableFileLogging("logs/debug.log", LogCategory::Debug);
	...
	void Log::DisableFileLogging(const std::string& filename)
	{
		...
		removeFromLogger(s_DebugLogger);
